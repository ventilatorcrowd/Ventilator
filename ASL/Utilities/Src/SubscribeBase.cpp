 ...
 */
bool CSubscribeBase::subscribe(GPIO_TypeDef * GPIO_port, uint16_t GPIO_pin, uint32_t pull_up_down
                                        ...
                                        , callbackFuncPtr_t deassertCallback, void * deassertArg)
{
    ...
    subscription.assertCallback = assertCallback;
    subscription.assertArg = assertArg;
    subscription.deassertCallback = deassertCallback;
    subscription.deassertArg = deassertArg;
    ...
    if (xSemaphoreTake(xSemaphore, portMAX_DELAY))
    {
        ...
        if ((-1) != nodeID)
        {
            subscriptionList.deleteNode((size_t)nodeID);
        }
...
/**\brief   Gets pointer to assert callback function.
 *
 * \param   listID  - subscription entry ID
 *
 * \return  assert callback pointer
 */
callbackFuncPtr_t CSubscribeBase::getAssertCallback(uint32_t listID)
{
    subscription_t * subscription = (subscription_t *)subscriptionList.findNode(listID);

    return subscription->assertCallback;
}

/**\brief   Gets pointer to assert callback msg.
 *
 * \param   listID  - subscription entry ID
 *
 * \return  assert callback msg pointer
 */
void * CSubscribeBase::getAssertCallbackMsg(uint32_t listID)
{
    subscription_t * subscription = (subscription_t *)subscriptionList.findNode(listID);

    return subscription->assertArg;
}

/**\brief   Gets pointer to de-assert callback function.
 *
 * \param   listID  - subscription entry ID
 *
 * \return  de-assert callback pointer
 */
callbackFuncPtr_t CSubscribeBase::getDeassertCallback(uint32_t listID)
{
    subscription_t * subscription = (subscription_t *)subscriptionList.findNode(listID);

    return subscription->deassertCallback;
}


/**\brief   Gets pointer to de-assert callback msg.
 *
 * \param   listID  - subscription entry ID
 *
 * \return  de-assert callback msg pointer
 */
void * CSubscribeBase::getDeassertCallbackMsg(uint32_t listID)
{
    subscription_t * subscription = (subscription_t *)subscriptionList.findNode(listID);

    return subscription->deassertArg;
}
